...
def solve_VRPTW(
    ...
    big_m: float,
    timelimit: float,
    premium_customers: np.ndarray = None
):
    """
    solve VRPTW with soft time window
    ...
    # Modified objective function to only apply late penalties to premium customers
    ...
    model.setObjective(
        ...
        quicksum(late_penalty_weight[i] * l[i] for i in C if premium_customers[i])
    )
    ...
    # Only apply to premium customers if needed (currently not used as per requirements)
    for i in C:
        for k in V:
            model.addConstr(s[i, k] + e[i] >= time_window[i, 0] * y[i, k])
    
    # Late arrival: s[i, k] - l[i] <= time_window[i, 1] + big_m * (1 - y[i, k])
    # Apply to all customers for tracking, but penalties only apply to premium customers in objective
    for i in C:
        for k in V:
